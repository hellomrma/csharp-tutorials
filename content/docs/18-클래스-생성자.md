---
title: 클래스의 생성자 (Constructor)
titleEn: Class Constructor
slugEn: 18-class-constructor
description: C#의 생성자를 학습합니다. 객체를 초기화하고 생성자 오버로딩을 사용하는 방법을 이해합니다.
...
### 예시 1: 무기 클래스 (일반 클래스)
```csharp
// MonoBehaviour가 아닌 일반 클래스에서는 생성자 사용 가능
public class Weapon
{
    public string name;
    public int damage;
...
// 사용
Weapon sword = new Weapon("철검", 50);
Weapon bow = new Weapon("장궁", 30, 1.5f);
Weapon defaultWeapon = new Weapon();  // 기본값 사용
```

### 예시 2: 플레이어 데이터 클래스
```csharp
// 게임 데이터를 저장하는 클래스 (MonoBehaviour 아님)
public class PlayerData
{
    ...
    public PlayerData(string name, int level)
    {
        this.name = name;
        this.level = level;
        this.maxHp = 100 + (level - 1) * 10;  // 레벨에 따라 최대 체력 증가
        this.hp = maxHp;
    }
    
    // 모든 정보를 받는 생성자
    public PlayerData(string name, int level, int hp, int maxHp)
    {
        this.name = name;
        this.level = level;
        this.hp = hp;
...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        // 생성자를 사용하여 플레이어 데이터 초기화
        playerData = new PlayerData("용사", 5);
        Debug.Log(playerData.name + " 레벨: " + playerData.level);
    }
}
```
